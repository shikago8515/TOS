...
1. 本文件是 AI 代码助手的执行约束，不是通用编程教材；只记录会影响任务执行、工程边界、安全、验证和交付的硬规则。
2. 通用建议必须写成可执行、可检查的项目规则。例如不要只写“代码简洁”，应明确“不做无关重构”“复用现有 API 层”“按改动范围运行检查”。
3. 更细的实现说明放在对应文档：前端规范见 `docs/frontend-engineering-standards.md`，后端模块与接口说明见 `tms-backend/README.md`，Electron 打包和发布说明见 `tms-electron-app/README.md`，命令矩阵见 `docs/engineering-entrypoints.md`，完整 AI 工作流见 `docs/tos-ai-workflow.md`。
4. 若本文件与更细文档冲突，先以当前源码、`package.json` scripts 和本文件的安全/发布/Git 边界为准，再修正文档漂移。
...
3. 新代码必须有清晰类型：TypeScript 禁止无理由 `any`；Python 新增函数必须写参数和返回值类型提示；API 请求、响应和文件处理结果不得用含糊结构绕过契约。
4. 旧恢复源码中超长函数、CommonJS 或历史结构不要求一次性重写；只在触及相关逻辑时做局部、可验证的改善。
5. 命名必须表达业务含义，避免 `temp`、`data2`、`handleThing` 这类无法说明职责的名称；已有历史命名只在触及相关逻辑且风险可控时局部改善。
6. 复杂业务逻辑、边界条件、安全处理和非显然实现添加中文注释；自解释代码不加空洞注释。
7. 生产代码不要新增 `console.log()` 或 `print()`。Electron/Node 脚本可保留现有命令行输出风格；后端应使用项目已有日志或诊断返回方案。
8. 文件、数据库连接、浏览器实例、临时目录、网络请求等资源必须及时释放，优先使用语言或项目已有的自动资源管理方式。
9. 所有外部输入进入文件路径、命令、数据库、HTML、日志或下载响应前，必须按场景做校验、转义或脱敏；不得新增内部异常细节直出给用户。
10. 新增依赖、大版本升级、公共抽象或跨模块复用前，先确认项目已有能力无法满足，并说明影响面；普通任务不引入新工具链。
11. 测试和验证按“日常开发验证分层”选择最接近的真实检查；不得为了省时间删除或弱化安全、契约、发布、部署关键测试。
...
6. 自己的测试服务器继续使用 `deploy-tos`；北京测试服务器固定使用本机 PowerShell 生成标准服务器包，再由本机 `scp`/SFTP 上传到 `/home/tosadmin/TOS/.deploy_uploads/`，最后在北京服务器执行包内 `deploy/apply-server-update.sh`。不要再使用或恢复 `scripts/server/deploy-to-beijing.sh` 这类一键 SSH 推送脚本。
