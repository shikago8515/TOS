<template>
  <el-card shadow="hover" class="main-card">
    <template #header>
      <div class="filter-bar">
        <div class="filter-group">
          <el-select v-model="classFilter" placeholder="班级筛选" clearable class="filter-select" style="width: 160px" size="small" :teleported="false">
          ...
          <el-input 
            ...
            size="small"
          />
          <el-select v-model="difficultyFilter" placeholder="难度" clearable class="filter-select" style="width: 100px" size="small" :teleported="false">
    ...
    <el-table 
      ...
      class="custom-table"
      :default-sort="{ prop: 'studentInfo.className', order: 'ascending' }"
    ...
    >
      ...
      <el-table-column prop="studentInfo.className" label="班级" width="100" sortable>
        <template #default="{ row }">
          <el-tag effect="plain" type="primary">{{ row.studentInfo?.className || '未分配' }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column label="学生信息" width="120">
        <template #default="{ row }">
          <div class="student-info-cell">
            <span class="student-name">{{ row.studentInfo?.realName || '未知' }}</span>
...
import { Search, User, DataLine, Edit, Delete, Download } from '@element-plus/icons-vue'
...
const emit = defineEmits(['selection-change', 'view-assignments', 'edit', 'delete', 'withdraw', 'page-change', 'size-change'])
...
const uniqueClasses = computed(() => {
  const classes = new Set(props.cases.map(c => c.studentInfo?.className).filter(Boolean))
...
const filteredCases = computed(() => {
  ...
  if (classFilter.value) {
    res = res.filter(c => c.studentInfo?.className === classFilter.value)
  }
  
  if (studentSearch.value) {
    const q = studentSearch.value.toLowerCase()
    res = res.filter(c => 
      String(c.studentInfo?.realName || '').toLowerCase().includes(q) ||
      String(c.studentInfo?.studentId || '').toLowerCase().includes(q)
    )
  }
  ...
  if (searchQuery.value) {
    ...
    res = res.filter(c => String(c.caseName || '').toLowerCase().includes(q))
  }
...
watch([classFilter, studentSearch, difficultyFilter, searchQuery, caseDateRange], () => {
  clearSelection()
})
