...
- CI 会设置 `PLAYWRIGHT_SKIP_BROWSER_DOWNLOAD=1`，并在 runner 内通过系统包安装 `nodejs`、`npm`、`python3`、`pip` 和 venv 支持；依赖安装与完整检查都会通过仓库内 `.venv` 暴露的 `PYTHON` 和 `PATH` 运行，避免在 Alpine runner 的 PEP 668 系统 Python 环境中直接安装后端依赖。当前远端检查只运行脚本级测试，不下载浏览器，也不做真实浏览器自动化 smoke。
