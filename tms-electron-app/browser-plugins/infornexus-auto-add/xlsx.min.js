...
// export { set_cptable }; // 注释掉 ES6 export，浏览器环境不支持 (Comment out ES6 export, browsers don't support it)
var DENSE = null;
var DIF_XL = true;
...
// export { set_fs }; // 注释掉 ES6 export，浏览器环境不支持 (Comment out ES6 export, browsers don't support it)
...
// export const version = XLSX.version; // 注释掉 ES6 export，浏览器环境不支持 (Comment out ES6 export, browsers don't support it)
// export { // 注释掉 ES6 export，浏览器环境不支持 (Comment out ES6 export, browsers don't support it)
// 	parse_xlscfb,
// 	parse_zip,
// 	readSync as read,
...
// 兼容浏览器环境：将导出的内容挂载到 window.XLSX (Browser compatibility: mount exports to window.XLSX)
if (typeof window !== 'undefined') {
	if (!window.XLSX) window.XLSX = {};
	window.XLSX.read = readSync;
	window.XLSX.write = writeSync;
	window.XLSX.readFile = readFileSync;
	window.XLSX.writeFile = writeFileSync;
	window.XLSX.utils = utils;
	window.XLSX.version = XLSX.version;
	window.XLSX.SSF = SSF;
	window.XLSX.CFB = CFB;
	// 将 XLSX 对象上的其他属性也复制过去 (Copy other properties from XLSX object)
	for (var key in XLSX) {
		if (XLSX.hasOwnProperty(key) && !window.XLSX.hasOwnProperty(key)) {
			window.XLSX[key] = XLSX[key];
		}
	}
}