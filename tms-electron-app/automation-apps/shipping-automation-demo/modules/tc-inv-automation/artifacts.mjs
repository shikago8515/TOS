...
async function writeTcInvResultWorkbook(targetPath, result, rows, xlsx) {
  ...
  const summaryRows = [
    ...
    { 字段: "ZEQS", 值: result.buildAdjustmentResult?.zeqs?.actualAmount || result.buildAdjustmentResult?.adjustments?.zeqsInputValue || "" },
    { 字段: "ZEQS Reason", 值: result.buildAdjustmentResult?.zeqs?.reason?.code || "" },
