...
DEFAULT_RELEASE_UPDATE_RECORDS: list[dict[str, Any]] = [
    ...
    _default_record(
        ...
        "新增新龙泰账号档案管理",
        "新龙泰 Shipping 自动化页面支持保存多个 Infor Nexus 登录账号档案，可按名称选择不同账号，并在下拉列表中直接删除不再使用的账号记录。",
    ),
    _default_record(
        "builtin-0.9.8-beta.3.27-improved-installer-and-update-center",
        "0.9.8-beta.3.27",
        "2026-06-18",
        "improved",
        "系统设置 / 安装包下载",
        "/settings",
        "优化安装包版本与更新检查",
        "设置页区分本机运行版本和服务器安装包版本，桌面端检查更新改为读取服务器安装包清单；完整安装包、轻量安装包和自动化助手安装包均保持带版本号输出。",
    ),
    _default_record(
        "builtin-0.9.8-beta.3.27-fixed-shipping-actions-and-credentials",
        "0.9.8-beta.3.27",
        "2026-06-18",
        "fixed",
        "万代 Shipping / 新龙泰 Shipping 自动化",
        "/web-automation",
        "修复 Shipping 动作选择与账号保留",
        "万代 Shipping 固定选择 Assign Equipment ID，新龙泰 Shipping 固定选择 Remove/Change Equipment ID；账号和密码中的 @@ 原样保存、回显和提交，不再被自动替换；Infor Nexus 登录失败和 Access Code 页面会给出中文弹窗提示。",
    ),
    _default_record(
        "builtin-0.9.8-beta.3.26-fix-desktop-embedded-automation-process-map",
        "0.9.8-beta.3.26",
        "2026-06-18",
        "fixed",
        "桌面安装包 / Shipping 自动化",
        "/web-automation",
        "修复桌面内置自动化执行器进程表缺失",
        "桌面端内置自动化启动器补齐进程管理表，修复启动 Shipping 执行器时报 ReferenceError: automationAppProcesses is not defined 的问题。",
    ),
    _default_record(
        "builtin-0.9.8-beta.3.25-remote-credentials-and-desktop-embedded-automation",
        "0.9.8-beta.3.25",
        "2026-06-18",
        "fixed",
        "网页自动化 / 桌面安装包",
        "/web-automation",
        "修复远程账号回显与桌面内置执行器启动",
        "服务器端 Infor Nexus 账号支持 Shipping 执行器 id 与各业务场景 id 共用远程数据库凭据；桌面端启动自动化时直接读取安装包内置 automation-apps，不再被本机旧小助手占用 3210 端口影响。",
    ),
    _default_record(
        "builtin-0.9.8-beta.3.24-remote-db-and-desktop-automation-package",
        "0.9.8-beta.3.24",
        "2026-06-18",
        "fixed",
        "桌面安装包 / 网页自动化",
        "/web-automation",
        "修复桌面安装包远程数据库与 Shipping 执行器识别",
        "桌面完整安装包默认使用远程服务器数据库读取账号、模板和运行记录；浏览器默认连接远程公开 API；Infor Nexus Shipping 相关场景兼容执行器 id 与业务场景 id 的账号读取，避免安装包中缺少账号或提示 Unknown automation app。",
    ),
    _default_record(
        "builtin-0.9.8-beta.3.23-fixed-helper-panel-and-shipping-launcher-message",
        "0.9.8-beta.3.23",
        "2026-06-18",
        "fixed",
        "自动化助手 / Shipping 自动化",
        "/settings",
        "修复小助手面板与 Shipping 执行器提示",
        "设置页版本读取改为使用公开安装包清单接口；打开小助手面板前会先检测本机小助手版本，旧版或未启动时给出中文提示；Shipping 执行器缺失时提示用户安装最新版完整包或小助手。",
    ),
    _default_record(
    ...
    _default_record(
        ...
        "自动核对列并批次化版本弹窗",
        "iPlex 双表核对删除可见列配置区，生成时自动识别固定业务列并在原位置显示核对异常表；版本更新弹窗改为按发布批次触发，可按模块集中展示更新内容，小修改默认不主动弹窗。",
    ),
    _default_record(
        "builtin-0.9.8-beta.3.24-fixed-iplex-skip-total-rows",
        "0.9.8-beta.3.24",
        "2026-06-18",
        "fixed",
        "iPlex 双表核对",
        "/iplex/dual-table-compare",
        "跳过 RC 总计行核对",
        "修复 iPlex 双表核对将 RC 表空 Key 总计行当作未匹配行处理的问题，总计行不再写入差值公式、不再标红，也不计入异常列表。",
    ),
    _default_record(
        "builtin-0.9.8-beta.3.23-improved-iplex-preview-and-red-row-marking",
        "0.9.8-beta.3.23",
        "2026-06-18",
        "improved",
        "iPlex 双表核对",
        "/iplex/dual-table-compare",
        "显示异常列表并标红 Excel 行",
        "iPlex 双表核对处理完成后在页面直接显示不一致或未匹配的 RC 行，并在导出的 Excel 中将差值不为 0 或未匹配的整行标红。",
    ),
    _default_record(
        "builtin-0.9.8-beta.3.22-fixed-iplex-dual-table-direction",
        "0.9.8-beta.3.22",
        "2026-06-18",
        "fixed",
        "iPlex 双表核对",
