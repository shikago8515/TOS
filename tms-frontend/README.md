...
`npm run dev` 与 `npm run dev:hybrid` 使用 Hybrid 模式：服务器共享数据接口读取远程后端，本地执行、Excel/PDF 处理和写入接口继续走本机 FastAPI `http://127.0.0.1:8000`。`npm run dev:local` 是纯本地后端模式；只有 `npm run dev:server` 使用 server mode，并读取 `.env.server` 的公开 `VITE_BACKEND_URL` 连接服务器后端。四者都占用 `5174`，切换模式前先停止旧 Vite 进程；根目录入口矩阵见 `../docs/engineering-entrypoints.md`。

当前端口：

- dev / dev:hybrid / dev:server / dev:local：`http://127.0.0.1:5174`
...
浏览器开发模式默认使用 Hybrid：共享数据接口连接服务器后端，本机执行与处理接口连接本机 FastAPI `http://127.0.0.1:8000`。只有显式运行 `npm run dev:local` 时才是全本机后端联调；只有 `npm run dev:server` 或部署在 `/tos` 路径下时才是全量服务器后端。
