...
export interface InfornexusMetaItem {
  label: string
  value: string
  wide?: boolean
}
...
export const infornexusStageMessage =
  'Infornexus 当前按外部子应用接入：TOS 只负责检测和启动，不拆分、不反编译、不内嵌它的压缩运行包。'

export const infornexusNotes = [
  '部署时需要保留完整 win-unpacked 目录内容，不能只复制 electron-app.exe 或 resources/app.asar。',
  '外部子应用会在独立窗口运行，登录态、缓存和运行时能力仍由原 Infornexus 包自己管理。',
  '如果后续要做成 TOS 内部页面，应按 Vue 3 + TypeScript 在 tms-frontend/src 里重建，而不是合并压缩 bundle。',
] as const
...
export function buildInfornexusMeta(
  moduleInfo: ExternalModuleInfo | null,
): InfornexusMetaItem[] {
  return [
    {
      label: '模块 ID',
      value: moduleInfo?.id || 'infornexus',
    },
    {
      label: '运行方式',
      value: '外部 Electron 子应用',
    },
    {
      label: '入口文件',
      value: moduleInfo?.executablePath || moduleInfo?.path || expectedInfornexusEntry,
      wide: true,
    },
  ]
}
