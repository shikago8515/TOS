<template>
  <section class="jane-bom-compare-page">
    <div class="card-section">
      <h2 class="section-title">{{ text('Jane-BOM核对') }}</h2>
      ...
      <FileRequirementGuide owner="Jane-BOM核对" mode="compact" />
      <FilePrecheckPanel :groups="fileGroups" />

      <div class="upload-grid">
        <FileUploadBox
          v-model:files="productionFiles"
          :label="text('T1 PRODUCTION 文件')"
          :hint="text('上传 1 个 T1 PRODUCTION.xlsx，输出会保留原表样式并标红差异。')"
          accept=".xlsx,.xlsm"
...
async function downloadResult(): Promise<void> {
  if (resultFile.value) {
    await downloadJaneBomCompareResult(resultFile.value)
  }
}
