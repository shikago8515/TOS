...
export interface AdidasMaterialsNote {
  icon: string
  text: string
}
...
export const adidasMaterialsCapabilities = [
  {
    label: '采集方式',
    value: '浏览器监听',
    desc: '外部 Edge/Chrome 登录后监听 Materials 接口响应',
  },
  {
    label: '保存策略',
    value: '自动落盘',
    desc: '默认每 2000 条保存 JSON 与 CSV 双格式',
  },
  {
    label: '恢复能力',
    value: '断点续传',
    desc: '本地保存去重 ID 与待保存批次，支持恢复',
  },
...
export const adidasMaterialsNotes: AdidasMaterialsNote[] = [
  { icon: 'window', text: '采集器运行在外部浏览器窗口中，不会在主应用里直接请求 adidas 页面。' },
  { icon: 'shield-check', text: '登录入口、账号权限和 Materials 列表页路径都按业务实际操作为准。' },
  { icon: 'download', text: '采集结束前先保存当前批次，避免剩余数据只在内存里。' },
]
