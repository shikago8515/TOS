<template>
  <div class="pad">
    ...
    <div v-if="!entry" class="pad-empty">
      ...
      <p>{{ text('请返回 Eric - Infornexus 页面重新选择场景。') }}</p>
        ...
        <main class="pad-main">
          <section class="pad-card pad-card--main">
            <div class="pad-card__head">
              ...
              <div>
                <strong>{{ text('请求下载 Invoice PDF') }}</strong>
                <small>{{ text('上传 Excel 后按 Invoice Number 发起请求下载到本机目录。') }}</small>
              </div>
            ...
            <div v-if="hasStoredCredentials" class="pad-note">
            ...
            <div class="pad-card__body">
              ...
              <div class="pad-row">
                <button class="pad-btn pad-btn--primary" type="button" :disabled="credentialSaving || !shippingUsername || !shippingPassword" @click="saveCurrentCredentials">
                  <AppIcon name="shield-check" />{{ credentialSaving ? text('保存中...') : text('保存登录账号密码') }}
                </button>
                <button class="pad-btn" type="button" :disabled="credentialClearing || !hasStoredCredentials" @click="clearCurrentCredentials">
            ...
            <div class="pad-card__body">
              <label class="pad-field">
                <span class="pad-field-head">
                  ...
                  <button class="pad-btn pad-btn--template" type="button" :disabled="templateDownloading" @click="downloadTemplate">
                ...
                >
                  ...
                  <template v-else>
                    ...
                    <div class="pad-drop__text">
                      ...
                      <small>{{ text('请包含 INVOICE NUMBER 和 STATUS 列') }}</small>
                    </div>
                  </template>
            ...
            <div class="pad-card__footer">
              <button class="pad-btn pad-btn--run" type="button" :disabled="!canRunPoDownload" @click="runPoAutoDownload">
                <AppIcon :name="sending ? 'loader' : 'download-cloud'" :class="{ 'pad-spin': sending }" />
                {{ sending ? text('执行中...') : text('上传 Excel 并下载 Invoice PDF') }}
              </button>
            </div>
            ...
            <transition name="pad-alert">
              <div v-if="statusText || sending" class="pad-status" :class="inlineStatusClass">
                ...
                <span>{{ text(statusLabel) }} · {{ statusText || text('等待上传 Excel 并填写下载保存目录。') }}</span>
              </div>
            </transition>
        ...
        <aside class="pad-dock">
          <section class="pad-dock-card">
            ...
            <div class="pad-dock-card__body">
              <button class="pad-btn pad-btn--primary pad-btn--full" type="button" :disabled="!canLaunchActiveApp" @click="startActiveApp(false)">
                <AppIcon name="play-circle" />{{ launching ? text('启动中...') : text('启动执行器') }}
              </button>
              <button class="pad-btn pad-btn--danger pad-btn--full" type="button" :disabled="!canStopActiveApp" @click="stopActiveApp">
                <AppIcon name="stop-circle" />{{ text('停止') }}
              </button>
              <button class="pad-btn pad-btn--full" type="button" :disabled="refreshing" @click="refreshExecutorState(false)">
                <AppIcon name="refresh-cw" :class="{ 'pad-spin': refreshing }" />{{ text('刷新状态') }}
              </button>
            </div>
          </section>
...
const steps = [
  { title: '上传 Excel 文件', desc: '读取 INVOICE NUMBER 和 STATUS 列生成下载清单。' },
  { title: '选择保存目录', desc: '文件会直接保存到用户电脑指定目录。' },
  { title: '请求下载优先', desc: '本机执行器优先使用登录态发起下载请求。' },
  { title: '查看下载结果', desc: '完成数量、失败 Invoice 和保存路径会返回页面。' },
]
...
const executorRunUrl = computed(() => {
  ...
  return base ? `${base}/api/run-po-auto-download-file` : ''
})
...
const progressCurrent = computed(() => {
  const values = Array.isArray(runProgress.value?.currentInvoiceNumbers)
...
async function runPoAutoDownload(): Promise<void> {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      statusText.value = appendFailureExamples(json?.message || `HTTP ${response.status}`, json)
      lastResult.value = { ok: false, message: statusText.value }
      messageTone.value = 'error'
      message.value = text('执行失败。')
      return
    }
...
async function createBackendRunRecord(file: File): Promise<AutomationRunRecord | null> {
  ...
  try {
    ...
    await recordWebAutomationEvent('po-auto-download-run-record-create-failure', {
