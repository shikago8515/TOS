...
function syncActiveRunViewFromHealth(): void {
  const activeRun = findInfornexusAutoAddActiveRun(executorHealth.value)
  if (activeRun) {
    ...
    statusText.value = inputFileName ? `执行器仍在处理 ${inputFileName}，请勿重复启动。` : text('Infornexus 自动搜索添加仍在后台运行，请勿重复启动。')
...
function findInfornexusAutoAddActiveRun(health: LocalExecutorHealth | null | undefined): LocalExecutorRun | null {
  return findLocalExecutorActiveRun(health, (run) => {
    const action = String(run.action || '').trim()
    const inputMode = String(run.inputMode || '').trim()
    return action === 'run-infornexus-auto-add-file' || inputMode === 'infornexus-auto-add'
  })
}
...
function bfi(rp: unknown, fr: string, fn: string): AutomationRunFileInput | null { const u = buildArtifactUrl(rp); if (!u) return null; return { url: u, fileRole: fr, fileName: fn } }
function buildArtifactUrl(rp: unknown): string { const np = String(rp || '').trim(); if (!np) return ''; if (/^https?:\/\//i.test(np)) return np; const bu = String(entry?.executorBaseUrl || '').replace(/\/+$/, ''); return bu ? `${bu}${np.startsWith('/') ? np : `/${np}`}` : '' }
...
async function runInfornexusAutoAdd(): Promise<void> {
  ...
  try {
    ...
    const raw = await res.text(); lastRawResponse.value = raw; const j = safeParseExecutorJson(raw); const executorMessage = readExecutorResponseText(j)
    ...
    if (j?.ok) { statusLabel.value = '成功'; statusText.value = `已完成 ${j.completedIdCount ?? 0}/${j.totalIdCount ?? '?'} 个 ID。`; lastResult.value = { ok: true, message: executorMessage || undefined }; messageTone.value = 'success'; message.value = text('执行完成。'); return }
