...
const activeUsername = computed(() => isShippingScenario.value ? shippingUsername.value : microsoftUsername.value)
const activePassword = computed(() => isShippingScenario.value ? shippingPassword.value : microsoftPassword.value)
...
async function refreshExecutorCredentials(): Promise<void> {
  ...
  try {
    ...
    if (username && !shippingUsername.value && isShippingScenario.value) {
