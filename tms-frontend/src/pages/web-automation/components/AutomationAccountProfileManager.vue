...
async function resolveCredentials(): Promise<ResolvedProfile> {
  ...
  const local = readLocalCredential(props.automationId, key)
  if (local) {
    const resolved = buildResolvedCredential(local)
    cache.resolvedByKey.set(key, resolved)
    applyResolvedCredential(key, resolved)
    return { username: formatUsername(local.username), password: local.password, accountKey: key }
  }
  ...
  if (!publicCredentials) {
    try {
      ...
      const fallback = readLocalCredential(props.automationId, key)
      if (fallback) {
        const resolved = buildResolvedCredential(fallback)
        cache.resolvedByKey.set(key, resolved)
        applyResolvedCredential(key, resolved)
        return { username: formatUsername(fallback.username), password: fallback.password, accountKey: key }
      }
      throw error
    }
  }
...
async function saveEditor(): Promise<void> {
  ...
  try {
    const cache = getCache()
    saveLocalCredential(props.automationId, accountKey, username, password)
    let saved: ExecutorCredentials
    try {
      ...
      emit('notice', {
        tone: 'warning',
        message: readErrorMessage(error, '已保存到本机，后端暂时无法同步。'),
      })
    }
...
async function deleteProfile(accountKey: string): Promise<void> {
  ...
  try {
    ...
    await clearExecutorCredentials(props.automationId, key).catch(() => null)
    const cache = getCache()
...
function saveLocalCredential(automationId: string, accountKey: string, username: string, password: string): void {
  if (typeof window === 'undefined' || !automationId || !username || !password) return
  const key = normalizeCredentialKey(accountKey)
  const store = readLocalCredentialStore()
  store[automationId] = store[automationId] || {}
  store[automationId][key] = {
    automationId,
    accountKey: key,
    username: formatUsername(username),
    password,
...
function removeLocalCredential(automationId: string, accountKey: string): void {
  ...
  const store = readLocalCredentialStore()
  const group = store[automationId]
  if (!group) return
  delete group[normalizeCredentialKey(accountKey)]
  writeLocalCredentialStore(store)
}

function readLocalCredential(automationId: string, accountKey: string): LocalCredentialRecord | null {
  if (!automationId) return null
  return readLocalCredentialStore()[automationId]?.[normalizeCredentialKey(accountKey)] || null
}

function listLocalCredentials(automationId: string): LocalCredentialRecord[] {
  if (!automationId) return []
  return Object.values(readLocalCredentialStore()[automationId] || {})
}
...
function writeLocalCredentialStore(store: LocalCredentialStore): void {
  ...
  try {
    ...
    // Local credential cache is best-effort; backend storage remains the shared source.
  }
}
