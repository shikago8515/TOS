        ...
        <div class="sa-left">
          ...
          <section class="sa-card sa-card--primary">
            ...
            <div v-if="shippingArtifactLinks?.resultExcelUrl" class="sa-card__ft sa-artifacts">
              <button class="sa-btn sa-btn--pri" @click="downloadShippingArtifact(shippingArtifactLinks.resultExcelUrl, 'xo-tc-inv-last-result.xlsx')">
                <AppIcon name="download" />{{ text('结果 Excel') }}
              </button>
              <button v-if="shippingArtifactLinks.failedPoExcelUrl && shippingArtifactLinks.failedRowCount > 0" class="sa-btn" @click="downloadShippingArtifact(shippingArtifactLinks.failedPoExcelUrl, 'xo-tc-inv-last-failed-rows.xlsx')">
...
const TC_INV_ENTRY_ID = 'xo-tc-inv-automation'
const PREVIEW_PDF_DIR_STORAGE_KEY = 'tos-xo-tc-inv-preview-pdf-directory'
...
const shippingExecutorRunUrl = computed(() => { const b = String(entry?.executorBaseUrl || '').replace(/\/+$/, ''); return b ? `${b}/api/run-xo-tc-inv-file` : '' })
const previewPdfDirectorySelectUrl = computed(() => { const b = String(entry?.executorBaseUrl || '').replace(/\/+$/, ''); return b ? `${b}/api/select-xo-tc-inv-pdf-directory` : '' })
...
const executorSupportsPreviewPdfDirectoryPicker = computed(() => Boolean(window.electronAPI?.selectDirectory || executorHealth.value?.capabilities?.xoTcInvPreviewPdfDirectoryPicker))
...
function findTcInvActiveRun(health: LocalExecutorHealth | null | undefined): LocalExecutorRun | null {
  return findLocalExecutorActiveRun(health, (run) => {
    const action = String(run.action || '').trim()
    const inputMode = String(run.inputMode || '').trim()
    return action === 'run-xo-tc-inv-file' || inputMode === 'xo-tc-inv-invoice-search'
  })
}
...
async function createBackendRunRecord(f: File): Promise<AutomationRunRecord | null> {
  ...
  try {
    ...
    console.warn('[xo-tc-inv-automation] backend run record create failed', error)
    return null
  }
}
async function finishBackendRunRecord(r: AutomationRunRecord | null, ok: boolean, msg: string, p: ExecutorResponsePayload | null): Promise<void> {
  if (!r?.runId) return
  try {
    await finishAutomationRunRecord(r.runId, ok ? 'success' : 'failed', msg || (ok ? 'completed' : 'failed'), p, collectResultFiles(p))
  } catch (error) {
    console.warn('[xo-tc-inv-automation] backend run record finish failed', error)
  }
}
function collectResultFiles(p: ExecutorResponsePayload | null): AutomationRunFileInput[] {
  ...
  return [bfi(u.resultExcelUrl, 'result_excel', 'xo-tc-inv-last-result.xlsx'), bfi(u.resultJsonUrl, 'result_json', 'xo-tc-inv-last-result.json'), bfi(u.failedPoExcelUrl, 'failed_rows_excel', 'xo-tc-inv-last-failed-rows.xlsx'), bfi(u.failedPoJsonUrl, 'failed_rows_json', 'xo-tc-inv-last-failed-rows.json')].filter((x): x is AutomationRunFileInput => Boolean(x))
}
...
async function runTcInv(): Promise<void> {
  ...
  try {
    ...
    if (j?.ok) { statusLabel.value = '成功'; statusText.value = executorMessage || 'TC INV 自动化执行完成。'; lastResult.value = { ok: true, message: executorMessage || undefined }; messageTone.value = 'success'; message.value = text('执行完成。'); return }
