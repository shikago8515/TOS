                ...
                <div class="sa-path">
                  <div class="sa-input-wrap sa-input-wrap--path">
                    ...
                    <input
                      ...
                      class="sa-input"
                      type="text"
                      :placeholder="text('例如：D:\\Downloads\\InforNexus\\VENT')"
...
const PREVIEW_PDF_DIR_STORAGE_KEY = 'tos-shipping-preview-pdf-directory'
...
const previewPdfDirectorySelectUrl = computed(() => { const b = String(entry?.executorBaseUrl || '').replace(/\/+$/, ''); return b ? `${b}/api/select-shipping-pdf-directory` : '' })
...
const executorSupportsPreviewPdfDirectoryPicker = computed(() => Boolean(window.electronAPI?.selectDirectory || executorHealth.value?.capabilities?.shippingPreviewPdfDirectoryPicker))
...
const activeShippingRun = computed(() => findShippingActiveRun(executorHealth.value))
...
function syncActiveRunViewFromHealth(): void {
  const activeRun = findShippingActiveRun(executorHealth.value)
  if (activeRun) {
    ...
    statusText.value = inputFileName ? `执行器仍在处理 ${inputFileName}，请勿重复启动。` : text('万代 Shipping 自动化仍在后台运行，请勿重复启动。')
...
async function selectPreviewPdfDirectory(): Promise<void> {
  ...
  if (window.electronAPI?.selectDirectory) {
    try {
      const result = await window.electronAPI.selectDirectory({
        title: text('选择 VENT Preview PDF 保存目录'),
  ...
  try {
    ...
    message.value = text('正在打开 VENT Preview PDF 保存目录选择器，请留意系统弹窗。')
    ...
    if (response.status === 404) {
      messageTone.value = 'warning'
      message.value = text('本机自动化助手缺少 VENT 目录选择接口，请同步最新自动化模块后重试，或手动填写完整本机路径。')
      return
    }
    if (result?.canceled) {
      messageTone.value = 'info'
...
function collectPreviewPdfArchiveFiles(p: ExecutorResponsePayload | null): AutomationRunFileInput[] {
  ...
  const add = (rawUrl: unknown, rawFileName: unknown, index: number): void => {
    ...
    seen.add(url)
    const fileName = String(rawFileName || '').trim() || `vent-preview-${index + 1}.pdf`
...
async function runShipping(): Promise<void> {
  ...
  try {
    ...
    const res = await fetch(shippingExecutorRunUrl.value, { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-Executor-Token': currentEntry.localExecutorToken }, body: JSON.stringify({ fileName: file.name, fileBase64: fb64, token: currentEntry.localExecutorToken, headless: !showBrowserView.value, ...cp, automationId: currentEntry.id, shipmentScanAction: 'assign-equipment-id', downloadPreviewPdf: true, previewPdfDownloadDirectory: previewPdfSaveDirectory.value.trim() }) })
